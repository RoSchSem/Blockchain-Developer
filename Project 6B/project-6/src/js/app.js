App = {
    ...
    handleButtonClick: async function(event) {
        ...
        switch(processId) {
            case 1:
                ...
                break;
            case 2:
                return await App.testItem(event);
                break;
            case 3:
            ...
            case 5:
                ...
                break;
            case 6:
                return await App.transportItem(event);
                break;
            case 7:
    ...
    testItem: function (event) {
        ...
        App.contracts.SupplyChain.deployed().then(function(instance) {
            return instance.testItem(App.UPC, {from: App.metamaskAccountID});
        }).then(function(result) {
            $("#ftc-item").text("Transaction: " + result['tx']);
    ...
    transportItem: function (event) {
        event.preventDefault();
        var processId = parseInt($(event.target).data('id'));

        App.contracts.SupplyChain.deployed().then(function(instance) {
            return instance.transportItem(App.UPC, {from: App.metamaskAccountID});
        }).then(function(result) {
            $("#ftc-item").text("Transaction: " + result['tx']);
            console.log('transportItem',result);
        }).catch(function(err) {
            console.log(err.message);
        });
    },
