...
contract('Flight Surety Tests', async (accounts) => {
  ...
  it('Paid airlines can register flights', async () => {
    ...
    try {
        await config.flightSuretyApp.registerFlight('RSS-001-20191001', {from: config.owner});
    }
    catch(e) {
        console.log(e);
    }
    let registeredFlight = await config.flightSuretyData.isFlightRegistered.call('RSS-001-20191001', {from: config.flightSuretyApp.address});
  ...
  it('Purchaser can buy insurance', async () => {
    ...
    try {
        await config.flightSuretyApp.buyInsurance('RSS-001-20191001', {from: purchaser, value: web3.utils.toWei('1', 'ether')});
    }
    catch(e) {
        console.log(e);
    }

    let insuranceDetails = await config.flightSuretyApp.getInsurance.call('RSS-001-20191001', purchaser, {from: config.flightSuretyApp.address});
  ...
  it('Purchaser can cancel insurance and be refunded', async () => {
    ...
    try {
        await config.flightSuretyApp.cancelInsurance('RSS-001-20191001', {from: purchaser});
    }
    catch(e) {
        console.log(e);
    }
  ...
  it('Change flight status and payout on insurance', async () => {
    ...
    try {
        await config.flightSuretyApp.registerFlight('RSS-001-20191002', {from: config.owner});
    }
    catch(e) {
        console.log(e);
    }
    ...
    try {
        await config.flightSuretyApp.buyInsurance('RSS-001-20191002', {from: purchaser2, value: web3.utils.toWei('1', 'ether')});
    }
    catch(e) {
        console.log(e);
    }
    ...
    try {
        await config.flightSuretyApp.ownerChangeFlightStatusCode('RSS-001-20191002', 20, {from: config.owner});
    }
    catch(e) {
        console.log('CHANGE_STATUS:', e);
    }

    let statusCode = await config.flightSuretyData.getFlightStatusCode.call('RSS-001-20191002', {from: config.flightSuretyApp.address});
    assert.equal(statusCode, 20, 'Flight Status Code not changed');

    // await config.flightSuretyApp.setInsurancePayout('RSS-001-20191002');

    let initialBalance = new BigNumber(await web3.eth.getBalance(purchaser2));
    console.log('Initial Balance:', web3.utils.fromWei(initialBalance.toString(), 'ether'));

    await config.flightSuretyApp.payoutInsurance('RSS-001-20191002', {from: purchaser2});

    let newBalance = new BigNumber(await web3.eth.getBalance(purchaser2));
    console.log('New Balance:', web3.utils.fromWei(newBalance.toString(), 'ether'));

    let insuranceDetails = await config.flightSuretyApp.getInsurance.call('RSS-001-20191002', purchaser2, {from: config.flightSuretyApp.address});
